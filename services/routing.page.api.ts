...
import { RoutingPageConstants } from '@/constants/routing.page.constants';
import type { DirectionsReq, RouteDTO, RouteStepDTO } from '@/dto/routing.page.dto';
...
export class RoutingPageAPI {
  static async getRoute(
    ...
    try {
      ...
      if (!response.ok) {
        console.error(
          `[RoutingPageAPI] OpenRouteService error: ${response.status}`,
        );
        return APIResponseFactory.ok(
          RoutingPageAPI.buildFallbackRoute(
            request.startLat,
  ...
  private static buildFallbackRoute(
    ...
    const distanceKm = GeoPageUtils.haversineKm(startLat, startLng, endLat, endLng);
